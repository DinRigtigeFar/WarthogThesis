...
png("/home/jfl323/pigs/cleanWarts/abbababa/mappedToPig/reapeatMasked/plots/testBoxplotDstatsDesertH3.png",width=800, height = 800)
par(mar=c(5,14,4,4))
boxplot(dstatsDesertOut$Z ~ dstatsDesertOut$poptree,
        boxwex=0.5,
        main="All individuals Dstat combinations setting Desert as outroup (H3) and Sus scrofa as H4",
        horizontal=T,las=2,ylim=c(-30,30),xlab="Z score",cex.axis=1.2,cex.lab=1.5)
abline(v=c(-3,3),lty=2)
dev.off()

my_plot <- boxplot(dstatsDesertOut$Z ~ dstatsDesertOut$poptree, plot=FALSE)
outs <- dstatsDesertOut[dstatsDesertOut$Z %in% my_plot$out,]
write.table(outs, file="Outliers.txt", sep="\t", row.names=FALSE)
